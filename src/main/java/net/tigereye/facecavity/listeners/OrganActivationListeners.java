...
public class OrganActivationListeners {
    private static Map<Identifier, BiConsumer<LivingEntity,FaceCavityInstance>> abilityIDMap = new HashMap<>();
    ...
    public static void register(Identifier id,BiConsumer<LivingEntity,FaceCavityInstance> ability){
        abilityIDMap.put(id,ability);
    }
    public static boolean activate(Identifier id, FaceCavityInstance cc){
    ...
    public static void ActivateCreepy(LivingEntity entity, FaceCavityInstance cc){
        ...
        FaceCavityUtil.destroyOrgansWithKey(cc,CCOrganScores.EXPLOSIVE);
        OrganUtil.explode(entity, explosion_yield);
        if(entity.isAlive()) {
            entity.addStatusEffect(new StatusEffectInstance(CCStatusEffects.EXPLOSION_COOLDOWN, FaceCavity.config.EXPLOSION_COOLDOWN, 0, false, false, true));
        }
    }

    public static void ActivateDragonBreath(LivingEntity entity, FaceCavityInstance cc){
        ...
        if(!entity.hasStatusEffect(CCStatusEffects.DRAGON_BREATH_COOLDOWN)){
            entity.addStatusEffect(new StatusEffectInstance(CCStatusEffects.DRAGON_BREATH_COOLDOWN, FaceCavity.config.DRAGON_BREATH_COOLDOWN, 0, false, false, true));
            cc.projectileQueue.add(OrganUtil::spawnDragonBreath);
        }
    }

    public static void ActivateDragonBombs(LivingEntity entity, FaceCavityInstance cc){
        //if(entity.world.isClient){
        //    return; //this is spawning entities, this is no place for a client
        //}
    ...
    public static void ActivateForcefulSpit(LivingEntity entity, FaceCavityInstance cc){
        //if(entity.world.isClient){
    ...
    public static void ActivateFurnacePowered(LivingEntity entity, FaceCavityInstance cc){
    ...
    public static void ActivateIronRepair(LivingEntity entity, FaceCavityInstance cc){
        float ironRepair = cc.getOrganScore(CCOrganScores.IRON_REPAIR) - cc.getFaceCavityType().getDefaultOrganScore(CCOrganScores.IRON_REPAIR);
        //test for ability
        if(ironRepair <= 0){
            return;
        }
        ...
        //success! heal target
        cc.owner.heal(cc.owner.getMaxHealth()*FaceCavity.config.IRON_REPAIR_PERCENT);
        entity.playSound(SoundEvents.ENTITY_IRON_GOLEM_REPAIR, .75f, 1);
        cc.owner.addStatusEffect(new StatusEffectInstance(CCStatusEffects.IRON_REPAIR_COOLDOWN, (int)(FaceCavity.config.IRON_REPAIR_COOLDOWN/ironRepair), 0, false, false, true));
        itemStack.decrement(1);

    }

    public static void ActivateGhastly(LivingEntity entity, FaceCavityInstance cc){
        //if(entity.world.isClient){
        //    return; //this is spawning entities, this is no place for a client
        //}
    ...
    private static void ActivateGrazing(LivingEntity entity, FaceCavityInstance cc) {
        ...
        if(ateGrass){
            ...
            if(entity.hasStatusEffect(CCStatusEffects.RUMINATING)){
                ...
                duration = (int)Math.min(FaceCavity.config.RUMINATION_TIME*FaceCavity.config.RUMINATION_GRASS_PER_SQUARE*FaceCavity.config.RUMINATION_SQUARES_PER_STOMACH*grazing,
                        ruminating.getDuration()+(FaceCavity.config.RUMINATION_TIME*FaceCavity.config.RUMINATION_GRASS_PER_SQUARE));
            }
            else{
                duration = FaceCavity.config.RUMINATION_TIME*FaceCavity.config.RUMINATION_GRASS_PER_SQUARE;
            }
            entity.addStatusEffect(new StatusEffectInstance(CCStatusEffects.RUMINATING, duration, 0, false, false, true));
        }
    }

    public static void ActivatePyromancy(LivingEntity entity, FaceCavityInstance cc){
        //if(entity.world.isClient){
        //    return; //we are spawning entities, this is no place for a client
        //}
    ...
    public static void ActivateShulkerBullets(LivingEntity entity, FaceCavityInstance cc){
        //if(entity.world.isClient){
        //    return; //we are spawning entities, this is no place for a client
        //}
    ...
    public static void ActivateSilk(LivingEntity entity, FaceCavityInstance cc){
