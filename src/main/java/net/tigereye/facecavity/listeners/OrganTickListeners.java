...
public class OrganTickListeners {
    ...
    public static void TickBuoyant(LivingEntity entity, FaceCavityInstance chestCavity){
        ...
        float buoyancy = chestCavity.getOrganScore(CCOrganScores.BUOYANT) - chestCavity.getFaceCavityType().getDefaultOrganScore(CCOrganScores.BUOYANT);
        if(buoyancy != 0)
        {
    ...
    public static void TickHealth(LivingEntity entity, FaceCavityInstance cc){
        if (cc.getOrganScore(CCOrganScores.HEALTH) <= 0
                && cc.getFaceCavityType().getDefaultOrganScore(CCOrganScores.HEALTH) != 0)
        {
            if(entity.world.getTime() % FaceCavity.config.HEARTBLEED_RATE == 0) {
                cc.heartBleedTimer = cc.heartBleedTimer + 1;
                entity.damage(CCDamageSource.HEARTBLEED, Math.min(cc.heartBleedTimer,cc.getFaceCavityType().getHeartBleedCap()));
            }
        }
        else{
    ...
    public static void TickFiltration(LivingEntity entity, FaceCavityInstance cc){
        ...
        if(cc.getFaceCavityType().getDefaultOrganScore(CCOrganScores.FILTRATION) <= 0){ //don't bother if the target doesn't need kidneys
            return;
        }
        float KidneyRatio = cc.getOrganScore(CCOrganScores.FILTRATION)/cc.getFaceCavityType().getDefaultOrganScore(CCOrganScores.FILTRATION);
        if(KidneyRatio < 1)
        {
    ...
    public static void TickHydrophobia(LivingEntity entity, FaceCavityInstance cc){
        ...
        if(hydrophobia <= 0                                                         //do nothing if the target isn't hydrophobic
            || cc.getFaceCavityType().getDefaultOrganScore(CCOrganScores.HYDROPHOBIA) != 0){   //do nothing if they are by default, otherwise endermen will spaz even harder
