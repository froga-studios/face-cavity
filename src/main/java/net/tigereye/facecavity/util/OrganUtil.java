...
import net.tigereye.facecavity.registration.CCEnchantments;
...
public class OrganUtil {
    ...
    public static void displayCompatibility(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
        ...
        else if (tag != null && tag.contains(FaceCavity.COMPATIBILITY_TAG.toString())
    ...
    public static void shearSilk(LivingEntity entity){
        FaceCavityEntity.of(entity).ifPresent(cce -> {
            if(cce.getFaceCavityInstance().opened){
                float silk = cce.getFaceCavityInstance().getOrganScore(CCOrganScores.SILK);
    ...
    public static boolean spinWeb(LivingEntity entity, FaceCavityInstance cc, float silkScore){
        int hungerCost = 0;
