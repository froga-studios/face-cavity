...
public class GeneratedFaceCavityType implements FaceCavityType {

    private Map<Identifier,Float> defaultOrganScores = null;
    private FaceCavityInventory defaultFaceCavity = null;
    ...
    public GeneratedFaceCavityType(){
    }
    ...
    public List<ItemStack> generateLootDrops(Random random, int looting) {
        ...
        if(random.nextFloat() < (FaceCavity.config.UNIVERSAL_DONOR_RATE + (FaceCavity.config.ORGAN_BUNDLE_LOOTING_BOOST*looting))*getDropRateMultiplier()) {
    ...
    public void setOrganCompatibility(FaceCavityInstance instance){
        ...
        if(!playerFaceCavity) {
            ...
            else if (random.nextFloat() < FaceCavity.config.UNIVERSAL_DONOR_RATE) {
                universalOrgans = 1 + random.nextInt(3) + random.nextInt(3);
            }
            //each attempt, roll a random slot in the facecavity and turn that organ, if any, compatible
            while(universalOrgans > 0){
                ...
                if(itemStack != null && itemStack != ItemStack.EMPTY && OrganManager.isTrueOrgan(itemStack.getItem())){
                    itemStack.removeSubNbt(FaceCavity.COMPATIBILITY_TAG.toString());
                }
                universalOrgans--;
            }
        }
